...
class Game:
    ...
    def _declare_winner(self):
        ...
        prompt(f"{self.human}: {self._result['human']}")
        prompt(f"{self.computer}: {self._result['computer']}")
        ...
        if self.result['human'] > self.result['computer']:
            ...
            prompt(f"{self.computer} has won the tournament",
                                                prefix_space= True)
        else:
...
name_of_game = "Rock Paper Scissor Lizard Spock"
config_path = "/Users/kirankm/Desktop/Launch_School_Python/PY120/projects/RPS/rps_config.json"
Game(name_of_game, config_path).play()